...
describe("ShadowDOMManipulator", () => {
	...
	it("should throw an error if image not found", () => {
		expect(() =>
			manipulator.replaceImage("nonexistent.jpg", "newSrc.jpg"),
		).toThrow("Image with the specified source not found.");
	});
	...
	it("should apply patterns on DOM changes", () => {
		...
		expect(manipulator.serializeShadowDOM()).toBe(
			'<div xmlns="http://www.w3.org/1999/xhtml">Test text</div>',
		);

		manipulator.addPattern("Test", "Replaced");
		expect(manipulator.serializeShadowDOM()).toEqual(
			'<div xmlns="http://www.w3.org/1999/xhtml">Replaced text</div>',
		);

		shadowRoot.innerHTML += "<div>Test new text</div>";
		expect(manipulator.serializeShadowDOM()).toEqual(
			'<div xmlns="http://www.w3.org/1999/xhtml">Replaced text</div><div xmlns="http://www.w3.org/1999/xhtml">Test new text</div>',
		);
		...
		expect(manipulator.serializeShadowDOM()).toBe(
			'<div xmlns="http://www.w3.org/1999/xhtml">Replaced text</div><div xmlns="http://www.w3.org/1999/xhtml">Replaced new text</div>',
		);
	...
	it("should remove pattern", () => {
		...
		manipulator.applyPatterns();
		expect(manipulator.serializeShadowDOM()).toBe(
			'<div xmlns="http://www.w3.org/1999/xhtml">Replaced text</div><div xmlns="http://www.w3.org/1999/xhtml">Test text</div>',
		);
	});
});
