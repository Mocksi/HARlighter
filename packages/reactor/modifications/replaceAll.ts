...
export class ReplaceAllModification extends AppliableModification {
  ...
  override modifiedElementRemoved(element: Element, mocksiId: string): boolean {
...
function walkTree(
  ...
  changer: (textNode: Node) => null | TreeChange,
): TreeChange[] {
...
): (node: Node) => null | TreeChange {
  const { flags, patternRegexp, replacement } = toRegExpPattern(pattern);
...
function replaceFirstLetterCaseAndPlural(
  value: string,
