...
export class ReactorMutationObserver {
  ...
  handleMutation(mutation: MutationRecord) {
    console.debug(
      `Mutation: ${mutation.type} added: ${mutation.addedNodes.length} removed: ${mutation.removedNodes.length}`,
    );
    ...
    do {
      ...
      if (node instanceof Element) {
        const applied = Array.from(
          this.reactor.getAppliedModifications(),
        ...
        for (const modification of applied) {
          ...
          for (const req of reqs.modifications) {
            if (this.reactor.doc && matchesSelector(node, req)) {
              ...
              applyModification(node, req, this.reactor.doc).then(
                (appliedModification) => {
        ...
        if (mocksiId) {
          for (const attributes of node.attributes) {
            if (attributes.name.startsWith("mocksi-modified-")) {
              const modificationId = attributes.name.substring(
                "mocksi-modified-".length,
              );
