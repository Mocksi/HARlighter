...
import { MOCKSI_ALTERATIONS, SignupURL, STORAGE_KEY } from "./consts";
import { initializeMckSocket, sendMckSocketMessage } from "./mckSocket";
import { apiCall } from "./networking";
import { getEmail, getLastPageDom, logout } from "./utils";
...
function sendData(request: Map<string, any>) {
  ...
  chrome.tabs.get(currentTabId, (tab) => {
    ...
    sendMckSocketMessage(btoa(encodedDataStr));
  });
}
...
function handleChatResponse(response: ChatResponse) {
  chrome.storage.local.get([STORAGE_KEY], (result) => {
    let messages = result[STORAGE_KEY] ? JSON.parse(result[STORAGE_KEY]) : [];
    const newMessage = {
      role: "assistant",
    ...
    chrome.storage.local.set({ [STORAGE_KEY]: JSON.stringify(messages) }, () => {
  ...
  chrome.runtime.onMessage.addListener(
	(
	  ...
	  if (request.message === "ChatResponse") {
		if (request.body && typeof request.body === 'object' && 'chat_message' in request.body) {
			handleChatResponse(request.body as unknown as ChatResponse);
			sendResponse({ message: request.message, status: "success" });
		  } else {
			sendResponse({ message: request.message, status: "error", detail: "Invalid ChatResponse body" });
		  }
		  return true;
	  }
